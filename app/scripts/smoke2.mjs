...
await step('credits: no fake purchase', async () => { await side('Crédits'); const b = page.getByRole('button', { name: 'Sur la version web' }); await b.nth(1).waitFor(); if ((await b.count()) < 6 || !(await b.first().isDisabled())) throw new Error('expected disabled paid plans and packs in claude.ai'); });
