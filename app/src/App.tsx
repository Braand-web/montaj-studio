...
function Shortcuts({ onClose }: { onClose(): void }) {
  ...
  const groups: [string, [string, string][]][] = [
    ...
    ['Design', [['⌘/Ctrl D', T('Dupliquer', 'Duplicate')], ['⌘/Ctrl C · V', T('Copier / coller', 'Copy / paste')], ['⌘/Ctrl G · ⇧ G', T('Grouper / dégrouper', 'Group / ungroup')], ['⌘/Ctrl A', T('Tout sélectionner', 'Select all')], [T('Suppr', 'Del'), T('Supprimer', 'Delete')], [T('Flèches · ⇧', 'Arrows · ⇧'), T('Déplacer de 1 / 10 px', 'Nudge 1 / 10 px')], [T('Maj + clic', 'Shift + click'), T('Sélection multiple', 'Multi-select')], [T('Maj + coin', 'Shift + corner'), T('Garder les proportions', 'Keep proportions')], [T('Double-clic', 'Double-click'), T('Modifier un texte', 'Edit a text')], ['Alt', T('Désactiver le magnétisme', 'Disable snapping')], ['⌘/Ctrl + · − · 0', T('Zoomer / dézoomer / ajuster', 'Zoom in / out / fit')], ['] · [', T('Avancer / reculer (⇧ : tout devant / derrière)', 'Forward / backward (⇧: to front / back)')], ['⇧H · ⇧V', T('Retourner horizontalement / verticalement', 'Flip horizontally / vertically')], ['T', T('Ajouter un titre', 'Add a title')], [T('Clic droit', 'Right-click'), T('Menu contextuel', 'Context menu')]]],
    [T('Vidéo', 'Video'), [[T('Espace', 'Space'), T('Lecture / pause', 'Play / pause')], ['S', T('Scinder le clip', 'Split clip')], [T('Suppr', 'Del'), T('Supprimer le clip', 'Delete clip')], ['M', T('Ajouter un marqueur', 'Add marker')], ['J · K · L', T('Reculer 1 s · lecture/pause · avancer 1 s (⇧ : 5 s)', 'Back 1 s · play/pause · forward 1 s (⇧: 5 s)')], ['⌘/Ctrl D', T('Dupliquer le clip', 'Duplicate clip')], [T('⇧ Suppr', '⇧ Del'), T('Supprimer et refermer le vide', 'Ripple delete')], [T('Début · Fin', 'Home · End'), T('Aller au début / à la fin', 'Go to start / end')], [T('Clic droit sur un clip', 'Right-click a clip'), T('Menu contextuel', 'Context menu')], ['← →', T('Image par image (⇧ : 1 s)', 'Frame by frame (⇧: 1 s)')], ['⌘/Ctrl + molette', T('Zoom de la timeline', 'Timeline zoom')]]],
